...
* 위 과정을 거칠경우, Glue 테이블을 갱신할 필요가 없어지므로, 파티션을 갱신하는 부분을 제거한다.
  ...
  -> S3에 일일 parquet을 유지한다면, DynamoDB에 TTL 1년정도를 걸어 25GB를 초과하지않도록 조정해야할 수 있다.

* 12-07에 Gemini Free tier 한도가 갑자기 제한되는 문제로인해 중요도/대/소분류판별등이 되지 않아 뉴스수집기 자체가 작동하지 않았다. -> Groq llama-3.3-70b-versatile 모델을 사용하도록 변경하였다. -> 여전히 제한에 막힌다;
